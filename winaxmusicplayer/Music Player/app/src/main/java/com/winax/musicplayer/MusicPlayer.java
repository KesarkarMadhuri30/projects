...
public class MusicPlayer {
    ...
    public static long createFavlist(final Context context, final String name) {
        if (name != null && name.length() > 0) {
            final ContentResolver resolver = context.getContentResolver();
            final String[] projection = new String[]{
                    MediaStore.Audio.PlaylistsColumns.NAME
    ...
    public static void addToFavlist(final Context context, final long[] ids, final long favlistId) {
        final int size = ids.length;
        final ContentResolver resolver = context.getContentResolver();
        final String[] projection = new String[]{
                "max(" + "play_order" + ")",
        };
        final Uri uri = MediaStore.Audio.Playlists.Members.getContentUri("external", favlistId);
        Cursor cursor = null;
        int base = 0;
        ...
        final String message = context.getResources().getQuantityString(
                R.plurals.NNNtrackstofav, numinserted, numinserted);
        Toast.makeText(context, message, Toast.LENGTH_SHORT).show();
    }
